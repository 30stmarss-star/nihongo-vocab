...
## 절대 규칙 — 정답은 하나뿐이어야 합니다
오답을 빈칸에 넣어도 자연스러운 문장이 되면 그 문항은 **실패**입니다. 학습자는 맞는 답을
고르고도 틀렸다는 소리를 듣게 됩니다.
- 나쁜 예: 「あの店の店員は若い＿です。」 + [女性/男性/子ども/家族]
  → 女性도 男性도 다 말이 됩니다. 문장이 답을 하나로 좁혀주지 못했습니다.
- 좋은 예: 「わからない言葉は＿で調べます。」 + [辞書/地図/切符/写真]
  → 「調べます」가 辞書 하나만 남깁니다.
문장 안에 **정답 하나만 남기는 단서**(함께 쓰이는 동사, 장소·행동·상황, 앞뒤 맥락)를 반드시
넣으세요. 사람·사물의 속성을 그냥 서술하는 문장(~は若い＿です, ~は大きい＿です)은
단서가 없으니 쓰지 마세요.
"why"에 나머지 세 보기가 왜 안 되는지 한국어 한 줄로 적으세요. 적으면서 "이것도 말이 되는데"
싶은 보기가 있으면, 그 보기를 바꾸거나 문장에 단서를 더해 다시 만드세요.

## 출제 규칙
- 학습자 급수에 맞는 쉬운 문법·어휘만 씁니다. 급수를 넘는 한자에는 무리하지 마세요.
- 오답은 **그럴듯해야** 합니다. 뜻만 보고 지워지는 보기는 안 됩니다.
  품사가 같고, 문장에 넣어도 문법적으로는 말이 되는 것으로. 단, 의미는 그 문장에서 틀려야 합니다.
- choices는 정확히 4개, answerIndex는 정답의 0-기반 위치. 정답 위치는 문항마다 섞으세요.
- **보기·문장은 전부 일본어로 씁니다.** 영어나 한국어 보기를 섞지 마세요
  (みなと를 "port"로, 女性을 "woman"으로 적는 식 절대 금지). ko 필드만 한국어입니다.
- **보기 표기를 통일하세요.** 보통 한자로 쓰는 단어는 한자로 적습니다(けが→怪我, たべる→食べる).
  한 보기만 가나로 적으면 그 자체가 힌트가 됩니다. 원래 가나로만 쓰는 말은 그대로 둡니다.
- 보기 4개는 서로 달라야 합니다(같은 말을 표기만 바꿔 두 번 넣지 마세요).
- cloze의 정답은 **주어진 그 단어**(또는 문맥에 맞게 활용한 꼴)여야 합니다. 유의어로 바꾸지 마세요.
- ko: 문장의 한국어 뜻. usage 유형은 정답 문장의 뜻.
- 단어마다 정확히 하나씩, 요청받은 단어 순서 그대로.
- 유형이 한쪽으로 쏠리지 않게 섞으세요.

## 출력 — 반드시 JSON 하나만 (앞뒤 다른 텍스트 금지)
{"items":[{"kanji":"辞書","kind":"cloze","sentence":"わからない言葉は＿で調べます。","ko":"모르는 단어는 사전으로 찾습니다.","choices":["辞書","地図","切符","写真"],"answerIndex":0,"why":"조사·확인하는 도구는 辞書뿐 — 地図는 길, 切符는 표, 写真은 사진이라 調べる와 안 붙는다."}]}`;
  ...
  try {
    ...
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        // '정답이 하나뿐인가'를 실제로 따져봐야 해서 low로는 부족했다(중복 정답 사고).
